    ...
    watch(
      ...
      () => {
        ...
        if (!props.range) {
          ...
          const value1 = props.modelValue[0]
            ? formatValue(props.modelValue[0])
            : formatValue(new Date());
          const value2 = props.modelValue[1]
            ? formatValue(props.modelValue[1])
            : formatValue(new Date());
        ...
        for (let token of tokens) {
          if (token.toLowerCase().includes("d")) {
            ...
            if (token.length <= 2) {
              ...
              let month = capitalize(
                shortMonths[new Date(newValue).getMonth()]
              );
              result += `${month}${separators[tokens.indexOf(token)] || ""}`;
            }
