...
export default defineComponent({
  props: {
    navLinks: {
      ...
      validator: (links) =>
        links.every(
          (link) => hasOwnProperty(link, "title") && hasOwnProperty(link, "to")
        ),
    },
    logo: {
      type: Object,
  ...
  render() {
    ...
    const renderUserNavLinks = () => {
      if (hasOwnProperty(this.user, "links")) {
        if (
          this.user.links.every(
            (link) =>
              hasOwnProperty(link, "title") && hasOwnProperty(link, "route")
          )
        ) {
            ...
            h(
              RouterLink,
              {
                ...
                class: mobileLinkClass(
                  userLink.route.name === this.$route.name
                ),
              },
        ...
        default: ({ open }) => [
          h(
            "div",
            {
              class: [
                "mx-auto px-4",
                this.inContainer ? "container" : "sm:px-6 lg:px-8",
              ],
            },
            [
